...
const { checkUploadRateLimit, checkUploadBytesRateLimit } =
...
describe("POST /api/batches", () => {
  ...
  it.skip("should create batch with valid files", async () => {
    ...
    const file1 = new File(["fake image data"], "photo1.cr2", {
      type: "image/x-canon-raw",
    });
    const file2 = new File(["fake image data"], "photo2.nef", {
      type: "image/x-nikon-raw",
    });
    ...
    const request = new NextRequest(
      new URL("http://localhost:3000/api/batches"),
      {
        method: "POST",
        body: formData,
      },
  ...
  it("should reject request with too many files", async () => {
    ...
    for (let i = 0; i < 51; i++) {
      const file = new File(["data"], `photo${i}.cr2`, {
        type: "image/x-canon-raw",
      });
