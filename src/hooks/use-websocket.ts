  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        console.log(
          `User not authenticated (HTTP ${response.status}), skipping WebSocket connection`,
        );
        return;
      }
      ...
      socketInstance.on(
        "project-update",
        (notification: ProjectUpdateNotification) => {
          setNotifications((prev) =>
            [notification as WebSocketNotification, ...prev].slice(0, 50),
      ...
      socketInstance.on(
        "processing-progress",
        (notification: ProcessingProgressNotification) => {
          setNotifications((prev) =>
            [notification as WebSocketNotification, ...prev].slice(0, 50),
          );
        },
