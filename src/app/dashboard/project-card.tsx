  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Status Badge */}
        <div
          className={`absolute top-3 ${viewMode === "list" ? "right-auto left-3" : "right-3"}`}
        >
          <div
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-[10px] font-medium backdrop-blur-md ${statusConfig[project.status as keyof typeof statusConfig].badge}`}
          >
        ...
        {/* Content */}
        <CardContent
          className={`p-4 ${viewMode === "list" ? "flex-1 flex flex-col justify-center min-w-0" : ""}`}
        >
          <div className="mb-2">
            <h3
              className="font-semibold text-sm text-foreground truncate group-hover:text-primary/80 transition-colors"
              title={project.name}
            >
              {project.name}
            </h3>
        ...
        <CardFooter
          className={`p-4 pt-0 flex justify-between items-center text-xs text-muted-foreground border-t border-white/5 ${viewMode === "list" ? "border-t-0 flex-row items-center gap-4" : ""}`}
        >
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent
              align="end"
              className="bg-popover border-border rounded-sm"
            >
