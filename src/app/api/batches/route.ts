...
export async function POST(request: NextRequest): Promise<NextResponse> {
  ...
  try {
    ...
    const uploadBytesLimitCheck = await checkUploadBytesRateLimit(userId, totalBytes);
    if (!uploadBytesLimitCheck.success) {
      ...
      return NextResponse.json(
        ...
        { status: 429, headers: { 'Retry-After': String(uploadBytesLimitCheck.retryAfter) } }
      );
    }
    ...
    for (const { file, mimeType } of validatedFiles) {
      ...
      logger.debug('Job enqueued', { jobId: createdJobId, batchId, filename: file.name });
    }
